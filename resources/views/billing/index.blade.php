                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group">
                                ...
                                <td class="px-4 py-3 align-middle font-medium text-slate-700 dark:text-slate-200">
                                    @php
                                        ...
                                        $carriedAmount = (float) $inv->carried_underpayment;
                                        $basePrice = $displayPrice - $carriedAmount;
                                        $custArrears = $arrearsByCustomer[$inv->customer_id] ?? [];
                                    @endphp
                                    @if($carriedAmount > 0)
                                        {{-- Show breakdown when there's carried underpayment --}}
                                        <div class="text-xs text-slate-500 dark:text-slate-400">
                                            Tagihan: Rp {{ number_format($basePrice, 0, ',', '.') }}
                                        </div>
                                ...
                                <td class="px-4 py-3 align-middle text-right">
                                    <div class="flex justify-end gap-2">
                                        ...
                                        @if($inv->amount_paid > 0 || $inv->status == 'paid')
                                            <button type="button" onclick="openPaymentDetail({{ $inv->id }})"
